...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchMovies = async () => {
      try {
        const response = await axios.get(`/api/movies/popular?page=${currentPage}`, {
          timeout: 1000 
        });   
  ...
  const handleWatchTrailerClick = async (id, media_type, movieTitle) => {
    ...
    try {
      const response = await axios.get(`/api/trailers/${media_type}/${id}`);
     ...
     <div className="search-container">
      ...
      <main id="main">
        {movies && movies.length > 0 ? (
          ...
          <div className="n">
         <h1 className="no-results"><Lottie animationData={n1}  /> </h1>
